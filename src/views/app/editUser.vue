<template>
  <Modal
    ...
    <Form ref="dataForm" :model="dataForm" label-position="left" :rules="ruleValidate" :label-width="100">
      ...
      <FormItem label="管理员" prop="admin" v-if="showDlg">
        <Checkbox v-model="dataForm.admin"></Checkbox>
      </FormItem>
      <FormItem label="角色" prop="userIds">
...
import {queryList , getuserlist , dataFrom , getrols , addList ,
setList , getappuserlist , userqueryList } from '@/api/appList'
...
export default class ComponentMain extends Mixins(ViewBase, UrlManager) {
  ...
  async dataFormSubmit(dataForms: any) {
    ...
    const mapid = (this.$refs.selection as any).getSelection().map((it: any) => it.id)
    const query = !this.id ? {
      ...this.dataForm,
      userIds: mapid
    } : {
  ...
  async doSearch() {
    ...
    try {
      ...
      const { data: {
        items: list,
      } } = await getrols(this.$route.params.appId)
      this.list = list
      //  获取用户详情
      const { data: {
        items: userlist,
